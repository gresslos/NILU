...
def SetRTM(UVS, ia, iacr, ACM3D=None, AMACD=None, ACMCOM=None, ACMRT=None, BMAFLX=None, input_dir='', iccloud=True,
    ...
    if source=='thermal':
        ...
        mc_photons = int(1e7)
        if 'AllLevels' in additional_spesifications: mc_photons = int(1e2);  UVS.inp['mc_minphotons']=mc_photons
        UVS.inp['mc_photons']=mc_photons 
        ####### INFO 21 x 21 grid ################
    ...
    for SceneName in SceneNames:
        ...
        if 'montecarlo' in rte_solver:
            ...
            if 'AllLevels' in additional_spesifications:
                shape = (ACMCOM.latitude_active.shape[0], Nx, Ny, ACMCOM.height_level.shape[0])
            #############
        else: 
        ...
        libRad.thermal_eup_std = np.zeros(shape)
        ...
        if SceneName == 'Orbit_07883D': #
            target_start_lat, target_end_lat = 57.2, 58
            ...
            if np_mpi == 1:
                ...
                while finished_workers < num_workers:
                    ...
                    if rte_solver == "montecarlo":
                        if 'AllLevels' in additional_spesifications:
                            nlev = r1.shape[-1]
                            idx = (ia, slice(None), slice(None), slice(None, nlev))
                        else: 
                    ...
                    for source in sources:
                        try: 
                            ...
                            if rte_solver=='montecarlo':
                                ...
                                ################ test what happens when remove this!! #########
